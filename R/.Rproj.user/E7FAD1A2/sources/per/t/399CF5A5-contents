...
for (specie in species){
 ...
 # only for trials 2019## data <-subset[!(row.names(subset) %in% c("902","935","939","941","943","945","951","952","916")),]
 ...
 for (year in years){
  ...
  for (i in 1:length(haul_y)){
   ...
   cat(paste0("Estract values from MaxEnt for year ", year,", ", species,"\n"))
  ...
  ### exctract values from asc BIMAC IDW #####
   cat(paste0("Estract values from BIMAC IDW for year ", year,", ", species,"\n"))
   ...
   cat(paste0("Estract values from BIMAC sd for year ", year,", ", species,"\n"))
